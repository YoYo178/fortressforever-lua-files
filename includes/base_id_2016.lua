...
function id_startup()
	...
	SetGameDescription( "Invade Defend 2016 Loade" )
	...
	--mostly just used for some consoleToAll stuff
	roundnumber = 1
	lastround = false
...
function startup()
	...
	-- really shouldn't ever be this long 
	set_cvar("mp_timelimit", 64) 
	ConsoleToAll("setting mp_timelimit: "..GetConvar( "mp_timelimit" ))
end
...
function round_end()
		...
		--if all 4 rounds have elapsed, call it quits
		if (rounds_elapsed >= 3) then
			lastround = true
			AddHudTextToAll("finalround_text", "FINAL ROUND", 20, od_hudstatusicony+5, 3, 0, 2)
			ConsoleToAll("declaring last round")
		end

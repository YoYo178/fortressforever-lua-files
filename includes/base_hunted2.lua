...
function startup()
	...
	-- set up team names
	SetTeamName( Team.kBlue, "The Hostage" )
	SetTeamName( Team.kRed, "The Good Guys" )
	SetTeamName( Team.kYellow, "The Bad Guys" )
	...
	SetPlayerLimit( Team.kYellow, 8 ) -- Only 6 assassins, but can we dynamically change this based on maxplayers and/or the current playercount?
	...
	team:SetClassLimit( Player.kSoldier, 3 )
	team:SetClassLimit( Player.kDemoman, 2 )
	team:SetClassLimit( Player.kMedic, 3 )
	team:SetClassLimit( Player.kHwguy, -1 )
	team:SetClassLimit( Player.kPyro, -1 )
	team:SetClassLimit( Player.kSpy, -1 )
	team:SetClassLimit( Player.kEngineer, -1 )
	...
	team:SetClassLimit( Player.kSoldier, 2 )
	team:SetClassLimit( Player.kDemoman, 1 )
	team:SetClassLimit( Player.kMedic, -1 )
	team:SetClassLimit( Player.kHwguy, -1 )
	team:SetClassLimit( Player.kPyro, -1 )
	team:SetClassLimit( Player.kSpy, 2 )
	team:SetClassLimit( Player.kEngineer, 1 )
	team:SetClassLimit( Player.kCivilian, -1 )
...
function hunted_escape_notification()
	BroadCastMessage( "The Hostage escaped!" )
...
function player_ondamage( player, damageinfo )
	...
	if player:GetTeamId() == Team.kBlue then
		...
		if attacker_is_buildable ~= true and weapon ~= "ff_weapon_sniperrifle" and weapon ~= "ff_projectile_rail" and weapon ~= "ff_weapon_crowbar" and weapon ~= "ff_projectile_dart" and weapon ~= "ff_weapon_knife" and weapon ~= "ff_weapon_spanner" and weapon ~= "ff_projectile_gl" then
	...
	-- hunted also has quad damage
		...
		if IsPlayer( attacker ) then
			...
			if attacker:GetTeamId() == Team.kBlue and player:GetTeamId() ~= HUNTED_ALLIES_TEAM then
				...
				ConsoleToAll( "The Hostage, " .. attacker:GetName() .. ", did a ass whoopin on" .. player:GetName() .. "!" )
...
function player_killed ( player_victim, damageinfo )
	...
	if player_victim:GetTeamId() == Team.kBlue then
		ConsoleToAll( "The Hostage, " .. player_victim:GetName() .. ", was annihilated!" )
		BroadCastMessage( "The Hostage was annihilated!" )
...
function player_spawn( player_entity )
	...
	if player:GetTeamId() ~= Team.kYellow then
		...
		player:Removemmo( Ammo.kGren2, 3 )
	end
...
function hunted_escape:ontouch( touch_entity )
	...
	if IsPlayer( touch_entity ) then
		...
		if player:GetTeamId() == Team.kBlue then
			...
			ConsoleToAll( "The Hostage, " .. player:GetName() .. ", is out of here!" )
			...
			AddSchedule( "close_escape_doors", 0.2, close_escape_doors )
...
escape_door_button = base_escape_door_button
escape_door_button = base_escape_door_button
escape_door_button = base_escape_door_button

...
function startup()
	...
	if AUTO_RESTOCK then
		AddScheduleRepeating( "Restock", RESTOCK_ITERATION_TIME, scheduleGiveConcs )
	end
end
...
function Freeze( tagger, tagged )
	...
	player_table[tagged:GetId()].no_unfreeze = true
	AddSchedule( "notagbacks"..tagged:GetId(), NO_TAG_BACK_TIME, EndNoUnFreeze, tagged:GetId() )
...
function UnFreeze( toucher, touched )
	...
	--player_table[touched:GetId()].no_freeze = true
	--AddSchedule( "notagbacks"..touched:GetId(), NO_TAG_BACK_TIME, EndNoFreeze, touched:GetId() )
...
function scheduleGiveConcs()
	local c = Collection()
	-- get all players
	c:GetByFilter({CF.kPlayers, CF.kPlayerScout})
...
function player_disconnected( player )
	table.remove(player_table, player:GetId())
end
	...
	for temp in c.items do
		...
		if player then
			if player_tagger:GetTeamId() == IT_TEAM then
				if player_table[player:GetId()].no_freeze then return end
				Freeze( player_tagger, player )
				return
			elseif player_tagger:GetTeamId() == FROZEN_TEAM then
				if player_table[player_tagger:GetId()].no_unfreeze then return end

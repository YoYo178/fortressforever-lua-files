...
-- piss off
function startup()
SetTeamName( Team.kBlue, "Blue Santaz" )
SetTeamName( Team.kRed, "Red Santaz" )
...
aardvarkpack = genericbackpack:new({
	...
	cells = 130,
	respawntime = 8,
	model = "models/johny-srka/pudding.mdl",
...
-----------------------------------------------------------------------------
--concmap secret headzy backpack
-----------------------------------------------------------------------------
...
function endzone:ontouch( touch_entity )
	...
	if IsPlayer( touch_entity ) then
		...
		ConsoleToAll( player:GetName() .. " has completed the secret Headzy concmap 3000 HD" )
		BroadCastMessage( player:GetName() .. " has completed the secret Headzy concmap 3000 HD" )

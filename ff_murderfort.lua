...
blue_spawndoor = respawndoor:new({team = Team.kBlue})
red_spawndoor = respawndoor:new({team = Team.kRed})
yellow_spawndoor = respawndoor:new({team = Team.kYellow})
green_spawndoor = respawndoor:new({team = Team.kGreen})
...
function murder_ball:ownertouchbase(player, ball)
	ball:Drop(BALL_RETURN_TIME, 0.0)
	self.status = 2
...
function murder_ball:dropitemcmd(owner_entity)
	if allowdrop == false then return end
	
	murder_ball:onownerdie(owner_entity)
end
...
spawn_antiball = trigger_ff_script:new({})

function spawn_antiball:ontouch(touch_entity)
	if IsPlayer(touch_entity) then
		local player = CastToPlayer(touch_entity)
		local ball = GetBallEntity(player)
		if ball ~= 0 then
			murder_ball:ownertouchbase(player, CastToInfoScript(ball))
		end
	end
end
...
-- block other teams from entering the base
clip_spawn_blue = trigger_ff_clip:new({ clipflags = {ClipFlags.kClipAllInfoScripts, ClipFlags.kClipPlayersByTeam, ClipFlags.kClipTeamRed, ClipFlags.kClipTeamYellow, ClipFlags.kClipTeamGreen, ClipFlags.kClipAllNonPlayers} })
clip_spawn_red = trigger_ff_clip:new({ clipflags = {ClipFlags.kClipAllInfoScripts, ClipFlags.kClipPlayersByTeam, ClipFlags.kClipTeamBlue, ClipFlags.kClipTeamYellow, ClipFlags.kClipTeamGreen, ClipFlags.kClipAllNonPlayers} })
clip_spawn_yellow = trigger_ff_clip:new({ clipflags = {ClipFlags.kClipAllInfoScripts, ClipFlags.kClipPlayersByTeam, ClipFlags.kClipTeamBlue, ClipFlags.kClipTeamRed, ClipFlags.kClipTeamGreen, ClipFlags.kClipAllNonPlayers} })
clip_spawn_green = trigger_ff_clip:new({ clipflags = {ClipFlags.kClipAllInfoScripts, ClipFlags.kClipPlayersByTeam, ClipFlags.kClipTeamBlue, ClipFlags.kClipTeamRed, ClipFlags.kClipTeamYellow, ClipFlags.kClipAllNonPlayers} })
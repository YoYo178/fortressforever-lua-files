...
-- Usual CTF shit - since we have no specific Rock2 speech, and the word system sucks.
function precache()
	-- precache sounds
...
function baseflag:touch( touch_entity )
	...
	if player:GetTeamId() ~= self.team then
		...
		SmartSpeak(player, "RK_TAKEKEY", "RK_GOTKEY", "RK_LOSTKEY")
		SmartMessage(player, "You have the Enemy Key!", "Your Team has the Enemy Key!", "The Enemy Team has your Key!")
...
function baseflag:onreturn( )
	...
	SmartTeamSpeak(team, "RK_KEYBACK", "RK_EKEYBACK")
	local flag = CastToInfoScript( entity )
...
function basecap:oncapture(player, item)
	...
	if self.team == Team.kBlue then
		SmartSpeak(player, "RK_BSCORE", "RK_BSCORE", "RK_BSCORE")
	else
		SmartSpeak(player, "RK_RSCORE", "RK_RSCORE", "RK_RSCORE")
	end
end
...
function detpack_trigger_single:onexplode( explosion_entity )
   if IsDetpack( explosion_entity ) then
	...
	if self.team == Team.kBlue then
		SmartTeamSpeak(GetTeam(self.team), "RK_BYARDOPEN", "RK_BYARDOPEN")
		BroadCastMessage ("Blue Yard has been Breached!")
		BroadCastSound("misc.buzwarn")
	else
		SmartTeamSpeak(GetTeam(self.team), "RK_RYARDOPEN", "RK_RYARDOPEN")
		BroadCastMessage ("Red Yard has been Breached!")
		BroadCastSound("misc.buzwarn")
	end
...
red_detpack_trigger_single = detpack_trigger_single:new({ team = Team.kRed, team_name = "red" })
blue_detpack_trigger_single = detpack_trigger_single:new({ team = Team.kBlue, team_name = "blue" })
green_detpack_trigger_single = detpack_trigger_single:new({ team = Team.kGreen, team_name = "green" })
yellow_detpack_trigger_single = detpack_trigger_single:new({ team = Team.kYellow, team_name = "yellow" })

...
blue_o2_only = function(self,player) return ((player:GetTeamId() == Team.kBlue) and ((player:GetClass() == Player.kSniper) or (player:GetClass() == Player.kPyro))) end
blue_d_only = function(self,player) return ((player:GetTeamId() == Team.kBlue) and (((player:GetClass() == Player.kScout) == false) and ((player:GetClass() == Player.kMedic) == false) and ((player:GetClass() == Player.kSpy) == false) and ((player:GetClass() == Player.kSniper) == false) and ((player:GetClass() == Player.kPyro) == false))) end

blue_ospawn = { validspawn = blue_o_only }
blue_ospawn2 = { validspawn = blue_o2_only }
...
red_slayer = KILL_KILL_KILL:new({ team = Team.kRed })
blue_slayer = KILL_KILL_KILL:new({ team = Team.kBlue })
sec_blue_slayer = KILL_KILL_KILL:new({ team = Team.kRed })
sec_red_slayer = KILL_KILL_KILL:new({ team = Team.kBlue })
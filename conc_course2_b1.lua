...
function player_spawn(player_entity)
	if IsPlayer(player_entity) then
		...
		-- player:RemoveLocation(player_entity:GetId())
		team = player:GetTeam()
...
conctimer_end = trigger_ff_script:new({timer = 0, pos = 0, bronzetime = 150, silvertime = 100, goldtime = 50, bronzepoints = 50, silverpoints = 100, goldpoints = 150})
...
function conctimer_start:onendtouch(touch_entity)
	local player = CastToPlayer(touch_entity)
	--player:AddHudTimer('timer1');
end
function conctimer_end:onendtouch(touch_entity)
	local player = CastToPlayer(touch_entity)
	--player:RemoveHudTimer('timer1');
end
...
function returnbutton:onuse( touch_entity )
	local player = CastToPlayer(touch_entity)
	ConsoleToAll('Player used return button')
	if playerStateTable[player:GetId()] ~= nil then 
...
function set_jumppos:onendtouch(touch_entity)
	...
	if playerStateTable[player:GetId()]['jump'] < self.pos then
		...
		-- SmartSound(touch_entity, "conc_course.checkpoint", "conc_course.checkpoint", "conc_course.checkpoint");
		-- SpeakPlayer(player, "conc_course.checkpoint")
		-- BroadCastMessageToPlayer(player, "Checkpoint!");
	end		
end
...
endmessage = messagetrigger:new({msg = 'Gratz! You have reached the end of the beta course!', location = 'The end!', id = 1})
...
conctimer1_end = conctimer_start:new({timer = 1, pos=27, bronzetime = 150, silvertime = 100, goldtime = 50, bronzepoints = 50, silverpoints = 100, goldpoints = 150});
